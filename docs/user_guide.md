...
- `[y]` 以此问题开跑；**回车**取消；**直接输入修正后的问题**则就地替换并重新确认
  （2026-09-15 实测教训：碎片输入如 `y` 不会被当成问题开跑，系统会要求重述）；
- 理解站若判定问题**不可解析**（缺少模型/物理量/计算要求），闭环在假设与计划
  之前直接拦下转人工并列出**需补充的逐项清单**——不会拿着垃圾问题机械往下跑。
...
- **计划文档**：每版计划自动写成 `<项目目录>/plans/plan_vN.md`，审批提示里
  给出路径——可以用编辑器直接打开改。注意：**绕过审批菜单直接改文件不会
  生效**，循环会提示"检测到会话外修改"，须按 `[e]` 走编辑回读（转录→critic
  →审批）管线才转录成新版计划；
...
- **y**：以你的身份（actor=HUMAN）批准落账——**回车不是批准**（会提示按键语义后重问，
  2026-09-15 实测：回车落进意见通道像"卡死"，单字符手滑会白烧一轮修订）；
- **s**：展开每步的 inputs/expected_outputs 细节再决定；
- **q**（或输入流结束/EOF/Ctrl+C）：放弃——**绝不默认批准**。

critic 判定为 revise 就送审时，菜单上方会列出它的主要问题——批准前先看；
正式批准后进入实验阶段（每步经 DSH 站点会话 + 数值计算，请留意 spinner 的进度叙述）。
