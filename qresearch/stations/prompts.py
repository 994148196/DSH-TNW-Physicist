...
计划要求：
- 计划宁简勿繁：第一版计划是**最小可信实验集**——通常不超过 6 个可执行步骤，
  单点先行、扫描只取小网格（2–3 个取值），先回答"算得通吗 + 与已知对照吻合吗"；
  扩大尺寸、加宽扫描留给后续轮次。一次性排满大网格会把预算烧光，
  且一旦失败连诊断信息都没有；
...
- 是否有逻辑跳跃或不可执行步骤；
- 是否存在明显更便宜的替代实验；
- 是否**过度规划**：第一轮就排大网格或大量同质步骤，而少数便宜实验即可回答
  当前问题——过度规划按 blocker 处理，要求削减到最小可信实验集；
...
- 若可引用实验区为"（本轮无已通过验证的实验）"，observations 与 interpretations
  都必须输出**空数组 []**（experiment_ids 也随之留空——每条 observation 必须挂
  实验id，而此刻没有任何 id 可挂），严禁填写 "none" 或任何占位符，并把
  "本轮无通过验证的实验"写入 uncertainties；
